...
times=sorted(set([i/32 for i in range(1537)]+[48-.001,.001]))
def sample(file):
 ...
 return out
source=sample('Stonehoof_Collision_r01.blend');baked=sample('Stonehoof_Collision_Baked_r01.blend')
...
endpoint=max((baked[48][n].translation-source[48][n].translation).length for n in baked[48])
