...
HERE=Path(__file__).resolve().parent;bpy.ops.wm.open_mainfile(filepath=str(HERE/'Stonehoof_Collision_r01.blend'))
...
times=sorted(set([i/8 for i in range(385)]+list(contacts.values())))
for f in times:
 scene.frame_set(math.floor(f),subframe=f-math.floor(f));bpy.context.view_layer.update();ev=mesh.evaluated_get(bpy.context.evaluated_depsgraph_get());m=ev.to_mesh();row={'frame':f,'mesh_min_z':min((ev.matrix_world@v.co).z for v in m.vertices),'wall_clearance_m':min((ev.matrix_world@v.co).y for v in m.vertices)-build['wall_y'],'legs':{},'length_error':0}
 for tag in contacts:
...
drift={};vertical={}
for tag in contacts:
 part=[s for s in samples if s['frame']>=16];origin=Vector(part[0]['legs'][tag]['sole']);drift[tag]=max((Vector(s['legs'][tag]['sole'])-origin).length for s in part)
...
report={'samples':len(samples),'maximum_hoof_target_error_m':{tag:max((s['legs'][tag]['target_error'],s['frame']) for s in samples) for tag in contacts},'post_landing_support_drift_m':drift,'minimum_wall_clearance_after_contact_m':min((s['wall_clearance_m'],s['frame']) for s in samples if s['frame']>=12),'skid_sole_z_ranges_m':vertical,'minimum_mesh_z_m':min((s['mesh_min_z'],s['frame']) for s in samples),'bone_length_error':max(s['length_error'] for s in samples),'max_rotation_step_eighth_frame':max_turn,'joins':build['joins'],'accepted_sources_unchanged':all(hashlib.sha256(Path(p).read_bytes()).hexdigest()==h for p,h in build['source_sha256'].items()),'owner_approved':False,'detail':samples}
