...
for f in [0,5,5.5,6,6.5,6.625,6.75,7,7.5,8]:
 ...
 print(json.dumps({'f':f,'legs':{t:{'hip':list(a.pose.bones['MCH_Upper_'+t].head),'knee':list(a.pose.bones['MCH_Lower_'+t].head),'foot':list(a.pose.bones['MCH_Lower_'+t].tail),'target':list(bpy.data.objects['CTRL_Hoof_'+t].location),'pole':list(bpy.data.objects['CTRL_Pole_'+t].location)} for t in ['front_right','hind_right']}}))
