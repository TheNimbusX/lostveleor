...
for o in list(bpy.data.objects):
    if o.name.startswith('CTRL_'):bpy.data.objects.remove(o,do_unlink=True)
bpy.context.view_layer.objects.active=arm;arm.select_set(True);bpy.ops.object.mode_set(mode='EDIT')
for b in list(arm.data.edit_bones):
    if not b.use_deform:arm.data.edit_bones.remove(b)
bpy.ops.object.mode_set(mode='OBJECT')
actions={};errors=[]
for action_name,start,end in [('AN_Stonehoof_Windup',0,30),('AN_Stonehoof_Launch',30,36),('AN_Stonehoof_WindupStart_Review',0,36)]:
...
full=actions['AN_Stonehoof_WindupStart_Review'];arm.animation_data.action=full
...
track=arm.animation_data.nla_tracks.new();track.name='Review only · preparation and launch';strip=track.strips.new(full.name,0,full);strip.action_frame_start=0;strip.action_frame_end=36;strip.extrapolation='NOTHING'
arm['runtime_actions']='AN_Stonehoof_Windup (1.0s), AN_Stonehoof_Launch (0.2s). Review strip combines both.'
arm['root_motion']='No object translation; preview travel removed. Body offsets only express weight transfer.'
scene.frame_set(0)
bpy.ops.wm.save_as_mainfile(filepath=str(HERE/'Stonehoof_WindupStart_Baked_r01.blend'))
report={'deforming_bones':len(names),'runtime_clips':{'AN_Stonehoof_Windup':1.0,'AN_Stonehoof_Launch':.2},'sample_step_frames':.25,'maximum_baked_joint_position_error_m':max(errors),'maximum_baked_rotation_error_degrees':max_angle_error,'maximum_quarter_frame_rotation_change_degrees':max_step,'maximum_step_at':max_step_at,'root_travel_removed':True,'scale_curves':False,'owner_approved':False}
