...
bpy.ops.wm.open_mainfile(filepath=str(HERE/'Stonehoof_Brake_r01.blend'))
...
actions={};errors=[]
for action_name,start,end in [('AN_Stonehoof_Brake',0,18)]:
...
full=actions['AN_Stonehoof_Brake'];arm.animation_data.action=full
...
for old in list(bpy.data.actions):
    if old!=full:bpy.data.actions.remove(old)
full.use_frame_range=True;full.frame_start=0;full.frame_end=18
arm.animation_data.action=None
track=arm.animation_data.nla_tracks.new();track.name='Open-edge brake · no arena travel';strip=track.strips.new(full.name,0,full);strip.action_frame_start=0;strip.action_frame_end=18;strip.extrapolation='NOTHING'
arm['runtime_actions']='AN_Stonehoof_Brake (0.6s); open-edge stop, final accepted stance.'
arm['root_motion']='No object translation; preview travel removed. Body offsets only express weight transfer.'
scene.frame_set(0)
bpy.ops.wm.save_as_mainfile(filepath=str(HERE/'Stonehoof_Brake_Baked_r01.blend'))
report={'deforming_bones':len(names),'runtime_clips':{'AN_Stonehoof_Brake':.6},'sample_step_frames':.0625,'maximum_baked_joint_position_error_m':max(errors),'maximum_baked_rotation_error_degrees':max_angle_error,'maximum_sixteenth_frame_rotation_change_degrees':max_step,'maximum_step_at':max_step_at,'root_travel_removed':True,'scale_curves':False,'owner_approved':False}
