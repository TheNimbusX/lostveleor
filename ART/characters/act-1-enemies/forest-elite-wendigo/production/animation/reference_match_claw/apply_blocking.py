...
a=bpy.data.actions.new('AN_ForestWendigo_Claw_Reference_Blocking');a.use_fake_user=True;r.animation_data.action=a
previous_q={}
for frame,data in d['frames'].items():
 ...
 r.pose.bones['pelvis'].location=x[:3]
...
for name,fr in [('STANCE',0),('LOAD',24),('OPEN_WINDUP',36),('SWEEP_START',50),('SWEEP_NEAR_CAMERA',52),('FOLLOW_THROUGH',57),('RECOVERY',69),('SETTLE',84)]:s.timeline_markers.new(name,frame=fr)
movie=HERE.parents[2]/'review'/'higgsfield-refs'/'claw.mp4'
if movie.exists():
...
s.frame_set(0)
bpy.ops.wm.save_as_mainfile(filepath=str(HERE/'Claw_Reference_Blocking.blend'))
s.render.resolution_x=720;s.render.resolution_y=720
out=HERE/'blocking';out.mkdir(exist_ok=True)
for frame in (() if globals().get('SKIP_STILLS') else (0,24,36,40,44,48,50,52,57,61,65,69,84)):
 s.frame_set(frame);s.render.filepath=str(out/f'pose_{frame:03d}.png');bpy.ops.render.render(write_still=True)

smooth=a.copy();smooth.name='AN_ForestWendigo_Claw_Reference_Spline';smooth.use_fake_user=True
...
bpy.ops.wm.save_as_mainfile(filepath=str(HERE/'Claw_Reference_Spline.blend'))
