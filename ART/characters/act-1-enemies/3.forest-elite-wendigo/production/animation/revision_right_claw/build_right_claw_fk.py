...
OUT = HERE / "Wendigo_RightClaw_FK_Probe.blend"
S = bpy.context.scene
S.render.fps = 30
...
DEFAULT = dict(px=0, py=0, pz=0, hip=0, lower_yaw=0, chest=0,
               ...
               rupper_pitch=0, rupper_yaw=0, rlower_pitch=0)
...
# FK takes over only the right arm. These angles use the same rest-space
# conversion as the original static pose study, whose contact silhouette
# stayed outside the torso. Body translation is restrained so feet remain
# visually weighted during the strike.
FK_STATIONS = {
    0: dict(rupper_pitch=0, rupper_yaw=0, rlower_pitch=0),
    3: dict(px=.015, py=-.02, pz=-.035, hip=-2, lower_yaw=0,
            chest=-4, pitch=-3, rclav_yaw=5,
            rupper_pitch=22, rupper_yaw=-18, rlower_pitch=-4),
    7: dict(px=.035, py=-.04, pz=-.07, hip=-5, lower_yaw=0,
            chest=-10, pitch=-6, rclav_yaw=15,
            rupper_pitch=65, rupper_yaw=-48, rlower_pitch=-18),
    11: dict(px=.05, py=-.04, pz=-.09, hip=-8, lower_yaw=0,
             chest=-18, pitch=-8, head_yaw=8,
             rclav_yaw=23, rclav_pitch=0,
             rupper_pitch=98, rupper_yaw=-72, rlower_pitch=-32),
    14: dict(px=.05, py=-.04, pz=-.10, hip=-9, lower_yaw=0,
             chest=-20, pitch=-9, head_yaw=9,
             rclav_yaw=25, rclav_pitch=0,
             rupper_pitch=105, rupper_yaw=-75, rlower_pitch=-35),
    15: dict(px=.05, py=-.04, pz=-.10, hip=-9, lower_yaw=0,
             chest=-20, pitch=-9, head_yaw=9,
             rclav_yaw=25, rclav_pitch=0,
             rupper_pitch=105, rupper_yaw=-75, rlower_pitch=-35),
    16: dict(px=.045, py=.01, pz=-.12, hip=-2, lower_yaw=0,
             chest=-10, pitch=-12, head_yaw=7,
             rclav_yaw=18, rupper_pitch=103,
             rupper_yaw=-45, rlower_pitch=-20),
    17: dict(px=-.02, py=.07, pz=-.13, hip=6, lower_yaw=0,
             chest=12, pitch=-15, head_yaw=1,
             rclav_yaw=0, rupper_pitch=96,
             rupper_yaw=25, rlower_pitch=0),
    18: dict(px=-.07, py=.10, pz=-.10, hip=18, lower_yaw=0,
             chest=48, pitch=-15, head_yaw=-12,
             head_pitch=4, rclav_yaw=-20,
             rclav_pitch=0, rupper_pitch=85,
             rupper_yaw=92, rlower_pitch=20),
    19: dict(px=-.09, py=.12, pz=-.10, hip=22, lower_yaw=0,
             chest=52, pitch=-16, head_yaw=-16,
             head_pitch=5, rclav_yaw=-23,
             rclav_pitch=0, rupper_pitch=80,
             rupper_yaw=105, rlower_pitch=25),
    21: dict(px=-.08, py=.10, pz=-.10, hip=21, lower_yaw=0,
             chest=48, pitch=-15, head_yaw=-13,
             rclav_yaw=-20, rupper_pitch=82,
             rupper_yaw=96, rlower_pitch=20),
    24: dict(px=-.04, py=.05, pz=-.06, hip=11, lower_yaw=0,
             chest=24, pitch=-9, head_yaw=-5,
             rclav_yaw=-9, rupper_pitch=55,
             rupper_yaw=55, rlower_pitch=12),
    27: dict(px=-.01, py=.02, pz=-.025, hip=3, lower_yaw=0,
             chest=7, pitch=-3, head_yaw=-1,
             rclav_yaw=-1, rupper_pitch=20,
             rupper_yaw=18, rlower_pitch=4),
    30: dict(rupper_pitch=0, rupper_yaw=0, rlower_pitch=0),
}
for frame, values in FK_STATIONS.items():
    STATIONS[frame].update(values)
...
def pose(frame, v):
    ...
    turn('R_arm_upper', (1,0,0), v['rupper_pitch'])
    turn('R_arm_upper', (0,0,1), v['rupper_yaw'])
    turn('R_arm_lower', (1,0,0), v['rlower_pitch'])
