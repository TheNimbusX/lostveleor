...
def fix_collar(i):
    co=verts[i].co
    if not (1.98<co.z<2.4 and abs(co.x)<.83 and abs(co.y)<.82):
        return False
    w=weight[i]
    ...
    allowable=.98*smoothstep(.10,.68,radius)*(1-smoothstep(2.08,2.34,co.z))
...
region={i for i in range(len(verts)) if 1.99<verts[i].co.z<2.33
        and abs(verts[i].co.x)<.83 and abs(verts[i].co.y)<.82}
...
dst=OUT/'ForestWendigo_Rig_WeightRevision_v3.blend'
...
(OUT/'weight_revision_v3_report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
print('WEIGHT_V3',json.dumps(report),flush=True)
