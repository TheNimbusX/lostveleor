...
# Runtime-заметки дописывает не этот конвейер: переносим их из прежнего манифеста как есть.
previous=json.loads((P/'asset_manifest.json').read_text(encoding='utf8')) if (P/'asset_manifest.json').exists() else {}
manifest={
 'revision':'2026-09-26-hit-reaction',
 ...
 'hit_seconds':validation['hit_seconds'],
 ...
 'notes':['Back opening rebuilt with skinned manifold surface; generated micro-flaps removed.','Petals rebuilt as thick watertight curved surfaces, three bend bones and smooth weights per petal.','Foot weights repaired; all clips sampled frame by frame against ground.','Fruit visual visibility is controlled by View from Sim shot events, not by bone scale.','Game.Sim owns aim, 0.2 second firing intervals, 2 second flight and impacts.','Hit: 11 frames (0.333 s), starts and ends in the rest pose; bud squash uses bone scale on bud only; View drives it by HitPhase, additively during Ranged_Attack.'],
