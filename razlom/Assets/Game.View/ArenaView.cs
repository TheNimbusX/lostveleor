...
namespace Game.View
{
    ...
    public sealed class ArenaView : MonoBehaviour
    {
        ...
        //
        // Меш v5 приходил ростом 0.98 — отсюда прежние 1.82. Меш v6 вернулся из
        // Mixamo примерно в сто раз мельче: в Blender оба рига одного видимого
        // размера, но старый при масштабе объекта 0.01, а новый при 1.0
        // (1.84/0.0179 = 103 по ширине, 0.382/0.00389 = 98 по глубине).
        //
        // Значение ниже взято по этому замеру. Точный рост печатает
        // «Разлом → Проверить Pelag v6 runtime» полем meshHeight — по нему
        // множитель и уточняется, если Пелаг всё ещё не того размера.
        public float WoleScale = 182f;
