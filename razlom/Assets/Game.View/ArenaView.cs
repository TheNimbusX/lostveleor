...
namespace Game.View
{
    ...
    public sealed class ArenaView : MonoBehaviour
    {
        ...
        // МНОЖИТЕЛЬ РОСТА, и его приходится пересчитывать под каждое новое тело.
        //
        // Tripo отдаёт модель в своём масштабе, а не в метрах, поэтому рост меша
        // в единицах каждый раз свой. Симуляция считает в метрах, художник задал
        // 1.78 м, отсюда:
        //
        //     WoleScale = 1.78 / рост меша в единицах
        //
        // Меш v5 приходил ростом 0.98 — отсюда прежние 1.82. Меш v6 приходит
        // ростом 0.018, и 1.78 / 0.018 даёт 98.9.
        //
        // ЧИСЛО ВЗЯТО ИЗ ВАЛИДАТОРА, А НЕ ИЗ BLENDER, и это важно. Прикидка по
        // габаритам в Blender дала отношение 100 и множитель 182 — Пелаг вышел
        // ростом 3.28 м. Ошибка была в том, что у одного рига мерился габарит
        // арматуры, у другого меша, а это разные вещи.
        //
        // «Разлом → Проверить Pelag v6 runtime» печатает meshHeight прямо из
        // bind pose. Меняешь тело — смотри туда, а не на глаз.
        public float WoleScale = 98.9f;
