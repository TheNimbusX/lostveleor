...
{
    /// <summary>
    /// Вопрос «Отправиться в забег?» у арки (владелец 24 сентября: «плашка ещё без UI»); с 26 сентября
    /// окно в материале «Дым и свет». Префаб Resources/UI/Prefabs/CampRiftConfirmWc; показ и ответ —
    /// CampRiftEntrance. Появляется мягко: затемнение и карточка с лёгким подъёмом; дым, текст и свет
    /// карточки проявляет её UiInkGroup — включение окна само запускает проявление, подъём и
    /// прозрачность здесь идут поверх него.
