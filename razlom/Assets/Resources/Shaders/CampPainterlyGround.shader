Shader "Game/Studies/Painterly Ground"
{
    Properties
    {
        ...
        _TurfWeight ("Turf texture weight", Range(0,1)) = 0.27
        ...
        _IsRiverBank ("River slope", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings { float4 positionCS : SV_POSITION; float3 positionWS : TEXCOORD0; float2 uv : TEXCOORD1; half fog : TEXCOORD2; float4 shadowCoord : TEXCOORD3; half3 normalWS:TEXCOORD4; float bank:TEXCOORD5; };
            ...
            Varyings Vert(Attributes v)
            {
                ...
                // Верхняя кромка берега продолжает горизонтальную землю без тёмного шва нормалей.
                if(_IsRiverBank>.5) o.normalWS=lerp(half3(0,1,0),o.normalWS,smoothstep(.04,.55,v.uv.y));
                o.uv=TRANSFORM_TEX(v.uv,_BaseMap); o.fog=ComputeFogFactor(p.positionCS.z);o.bank=v.uv.y;
                return o;
            }
            half4 Frag(Varyings i) : SV_Target
            {
                ...
                if(_IsSurface>.5)
                {
                    ...
                    half3 turfA=SAMPLE_TEXTURE2D_BIAS(_TurfTex,sampler_TurfTex,p/5.2,_DetailSoftness).rgb;
                ...
                if(_IsRiverBank>.5)
                {
                    // У верхней кромки остаётся та же мировая покраска и освещение, что у земли лагеря.
                    float wet=saturate(i.bank);
                    float shore=smoothstep(.18,.83,wet+(Noise(p*4)-.5)*.20);
                    albedo=lerp(albedo,dirt*lerp(1,.86,smoothstep(.8,1,wet)),shore);
