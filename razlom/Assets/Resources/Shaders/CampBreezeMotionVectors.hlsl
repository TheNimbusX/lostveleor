...
#pragma vertex CampBreezeMotionVertex
...
// Прошлая фаза ветра нужна TAA: стандартный URP знает только о движении объекта целиком.
Varyings CampBreezeMotionVertex(CampBreezeAttributes input)
{
    UNITY_SETUP_INSTANCE_ID(input);
    ...
    #if _ALPHATEST_ON || APPLICATION_SPACE_WARP_MOTION_TRANSPARENT
        original.uv = input.uv;
    #endif
    Varyings output = vert(original);
    float4 previous = float4(CampBreezePosition(input.positionOS.xyz, input.bend, _CampBreezePreviousTime), 1);
