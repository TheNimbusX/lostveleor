...
namespace Game.Sim
{
    ...
    public sealed class LocationDefinition
    {
        ...
        public LocationDefinition(int id, ModuleSet modules, RiftLevelSettings[] levels, int maxModules = 64, bool completeAtEnd = false)
        {
            ...
            {
                if (levels[i].TargetModules < 2 || (levels[i].ArenaSize > 0 ? 19 : levels[i].NaturalGlade ? GladeLayout.RequiredModules(levels[i].TargetModules, levels[i].Boss) : levels[i].TargetModules + levels[i].MaxLoops) > maxModules)
