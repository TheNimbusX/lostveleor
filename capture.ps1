...
if ($needsBuild) {
    ...
    foreach ($folder in 'Assets', 'Packages', 'ProjectSettings') {
        ...
        robocopy (Join-Path $project $folder) (Join-Path $shadow $folder) /MIR /NFL /NDL /NJH /NJS /NP | Out-Null
...
if ($Video) {
    ...
    $movieName = if ($Skill -ne '') { "pelag_${Skill}_1080p60.mp4" } else { 'pelag_whirlwind_1080p60.mp4' }
